...
import de.sciss.synth.proc.{Server, Artifact, Scan, Grapheme}
...
object SearchStepAlgorithm {
  ...
  def apply(channel: Channel, server: Server, span: Span, group: ProcGroup, hidden: AudioArtifact)
    ...
    val posSegms = group.intersect(span).toIndexedSeq.flatMap { case (_, seq) =>
      seq.flatMap { timed =>
        val p = timed.value
